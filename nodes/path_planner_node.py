...
class PathPlannerNode:
    def __init__(self):
        ...
        self.subOdom = rospy.Subscriber("/test/odom", Odometry, self.callback_Odom)
        self.subOdom = rospy.Subscriber("/test/fallback_odom", Odometry, self.callback_Odom)
        ...
        if self.pathGlobalGen:
            ...
            try:
                if len(maskTrueInd[0]) > 0:
                    ...
                    self.searchSA_replan = AStarSearch(self.squareGridGraphTotal, startPoint, goalPoint, h_type='euclidean', inflation=5.0, visualize=rc.visualize)
...
#		    self.publish_path(newSegment)
    ...
    def get_plan(self):
        ...
        self.searchSA = AStarSearch(self.squareGridGraphStatic, odomFilt, goalFilt, h_type='euclidean', inflation=1.0, visualize=rc.visualize)
